...
Python itself normally exists just to run programs that you have written, but it can run as a program in its own right, allowing you to explore the language by typing commands which it then executes for you. In this course, we will focus solely on what you can do at the Python prompt, in this case the one that is available in the Google Colab IDE (Integrated Development Environment).

In reality, running full programs with Python, standalone and outside of Google Colab, is as simple as copying and pasting the code that you have written in Colab into a text file, saving it with the `.py` file extension and then executing it.

The main way you interact with Python in Colab is through **cells** (the box in the screenshot below that says "Start coding..."). You type Python commands into a code cell and run it by pressing **Shift+Enter** (which runs the cell and moves to the next one) or **Ctrl+Enter** (which runs the cell and stays). The output appears directly below the cell. You can have as many cells as you like in a notebook, and they all share the same Python session.

![Google Colab interface](fig/01_colab_interface.png){alt="Screenshot of the Google Colab interface showing an empty cell"}
...
The Python shell (which is what you are using now) is a great way to experiment with Python. It’s something that we return to every time we find ourselves thinking “I wonder what happens if you do this?” or “what’s the best way to do that?”. You can’t break anything by trying something and if you make a mistake at least you will get an error message that you might be able to decipher.

You are not limited to what you can do at the prompt. You can load modules, look at them to see what they do, play with them. The first versions of the bar charts in Episode 4 were all produced at the Python prompt, which enabled us to tweak them to see exactly how we wanted them before writing the program to produce the whole figure. This meant we could see how they looked after every command and get them looking just as we wanted. We could also check the documentation for the modules we wanted to use by typing `help()` and then the name of a function or module. There is a lot of information in there and you will find yourself using it again and again.

The only thing you can’t do at the interactive prompt, though, is save what you have done. In Colab, you can add new cells to write and run multi-line programs, and your notebook is automatically saved to Google Drive; we’ll see how to do that in Episode 2. If you ever want to use Python on your own computer instead, we recommend installing Anaconda Python, which comes with almost all of the modules you will ever need pre-installed (and makes it simpler for you to install others). Anaconda is available for Windows, Mac and Linux, and is worth installing if you plan to use more complex modules like numpy and matplotlib.
